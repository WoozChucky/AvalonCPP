...
av::entities::Enemy::Enemy(const sf::Vector2f l_size, const Class l_class) 
	: m_body_(l_size), m_class_(l_class), m_healthbar_(sf::Vector2f(- 500.f, - 500.f))
{
	//BODY
...
sf::Vector2f av::entities::Enemy::getPosition()
{
	return this->m_body_.getPosition();
}

sf::Vector2f av::entities::Enemy::getSize()
{
	return this->m_body_.getSize();
}

void av::entities::Enemy::TakeDamage(sf::Uint32 l_damage)
{
...
bool av::entities::Enemy::Collide(Player& l_entity)
{
	//TODO
	return false;
}
...
{
	if ( ( (l_entity.getGlobalBounds().left > getPosition().x) and (getPosition().x + getSize().x > l_entity.getGlobalBounds().left) ) ||
        ((l_entity.getGlobalBounds().left + l_entity.getGlobalBounds().width > getPosition().x) and ((getPosition().x + getSize().x) > l_entity.getGlobalBounds().left + l_entity.getGlobalBounds().width))) {
            if ((getPosition().y < l_entity.getGlobalBounds().top) and (getPosition().y + getSize().y > l_entity.getGlobalBounds().top)){
...
bool av::entities::Enemy::IsAlive(){
