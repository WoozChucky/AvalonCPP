...
av::Healthbar::Healthbar(const sf::Vector2f l_position)
...
void av::Healthbar::Update(float timestep) const
{

}

void av::Healthbar::Render(sf::RenderWindow& l_window) const
{
    l_window.draw(this->m_current_health_);
    l_window.draw(this->m_missing_health_);
}

void av::Healthbar::SetPosition(const sf::Vector2f l_position)
{
    this->m_current_health_.setPosition(l_position.x, l_position.y - HP_HEIGHT);
}

bool av::Healthbar::Minus(const sf::Uint32 l_damage)
{
    if (this->m_health_ - static_cast<int>(l_damage) <= 0) {
        return false; // Entity is 'dead'
    }

    const auto current_health_position = sf::Vector2f(((this->m_health_ - l_damage)) * HP_WIDTH / 100, HP_HEIGHT);
