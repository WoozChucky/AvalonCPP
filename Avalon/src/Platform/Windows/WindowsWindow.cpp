...
namespace Avalon {
	...
	{
		AV_CORE_ERROR("GLFW Error ({0}): {1}", error, description);
	}
	...
	*/
	...
	void WindowsWindow::Init(const WindowProps& props)
	{
		...
		AV_CORE_INFO("Creating window {0} ({1}, {2})", props.Title, props.Width, props.Height);

		if (s_GLFWWindowCount == 0)
		{
			int success = glfwInit();
			AV_CORE_ASSERT(success, "Could not initialize GLFW!");
